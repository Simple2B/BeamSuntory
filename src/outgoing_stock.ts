...
const modalEditOptions: ModalOptions = {
  ...
  onHide: () => {
    console.log('modal is hidden');
    const tableShipRequestBody = document.querySelector(
      '#table-outgoing-stock-body-edit',
    );
    while (tableShipRequestBody.firstChild) {
      tableShipRequestBody.removeChild(tableShipRequestBody.firstChild);
    }
...
function editShipRequest(shipRequest: IShipRequest, store: IStore) {
  ...
  let div: HTMLDivElement = document.querySelector(
    '#outgoing-stock-edit-order-number',
  );
  div.innerHTML = shipRequest.order_numb;
...
function createOutgoingStockItemTable(
  ...
  currentCartItems.forEach((product, index) => {
    ...
    tableShipRequestItem.innerHTML = `
        ...
        <td scope="row" class="p-4 text-base font-normal text-gray-900 whitespace-nowrap dark:text-white">
          <div class="pl-3">
            <div class="cart-item-product-name text-base font-semibold" data-target-product-id="${
              product.id
            }">${product.name}</div>
          </div>
        </td>
        ...
        <td class="p-4 text-base font-normal text-gray-900 whitespace-nowrap dark:text-white">
          <div class="pl-3">
            <div class="cart-item-group text-base font-semibold">${
              product.group
            }</div>
          </div>
        </td>
        <td class="p-4 text-base font-normal text-gray-900 whitespace-nowrap dark:text-white">
          <div class="pl-3">
            <div class="cart-item-quantity text-base font-semibold">${
              product.quantity
            }</div>
        </div>
        </td>
    ...
    if (typeModal === 'edit') {
      ...
      const selectWarehouse: HTMLInputElement =
        tableShipRequestItem.querySelector(
...
function setProducts() {
  ...
  const outgoingStockProductItem = document.querySelectorAll(
    '.cart-item-product-name',
  );
  const outgoingStockQuantityItems = document.querySelectorAll(
    '.cart-item-quantity',
  );
  const outgoingStockWarehouses = document.querySelectorAll(
    '.outgoing-stock-edit-warehouse-name',
  );
  ...
  for (let i = 0; i < outgoingStockProductItem.length; i++) {
    const outgoingStockProductNameItem = outgoingStockProductItem[
      i
    ] as HTMLSelectElement;
    const inboundOrderAddQuantity = outgoingStockQuantityItems[
      i
    ] as HTMLSelectElement;
    const outgoingStockGroupName = outgoingStockGroupItems[
      i
    ] as HTMLSelectElement;
    const outgoingStockWarehouse = outgoingStockWarehouses[
      i
    ] as HTMLSelectElement;

    const product = {
      product_id: Number(
        outgoingStockProductNameItem.getAttribute('data-target-product-id'),
      ),
