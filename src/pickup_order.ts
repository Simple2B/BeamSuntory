...
// ----view modal-----
const viewPickupOrderButtonElements = document.querySelectorAll(
  '.pickup-order-view-button',
);
viewPickupOrderButtonElements.forEach(e =>
  e.addEventListener('click', () => {
    ...
    let div: HTMLDivElement = document.querySelector(
      '#pickup-order-view-order-number',
    );
...
function createPickupOrderItemTable(shipRqst: IShipRequest, typeModal: string) {
  const tableShipRequestBody = document.querySelector(
    `#table-pickup-order-body-${typeModal}`,
  );
  ...
  currentCartItems.forEach((product, index) => {
    ...
    if (typeModal === 'edit') {
      ...
      const selectWarehouse = tableShipRequestItem.querySelector(
        `#pickup-order-${typeModal}-warehouse-name`,
      );
