...
const globalFilterMasterGroup = JSON.parse(
  sessionStorage.getItem('globalFilterMasterGroup'),
);
...
if (globalFilterMasterGroup && globalFilterMasterGroup.length !== 0) {
  ...
  for (const masterGroupName of globalFilterMasterGroup) {
    ...
    if (!isGroupExist) {
      ...
      const productItemTrs = document.querySelectorAll(
      ...
      productItemTrs.forEach((productItem: HTMLTableRowElement) => {
        ...
        const productFilterName =
          productMgGGlobal[productName][masterGroupName] || '-';
        const productFilterTd = document.createElement('td');
        productFilterTd.setAttribute(
          'id',
  ...
  checkbox.addEventListener('change', e => {
    ...
    if (isActive) {
      ...
      sessionStorage.setItem(
        'globalFilterMasterGroup',
        JSON.stringify(filterMasterGroup),
      );
      const isGroupExist = document.querySelector(
        `#product-table-filter-master-group-${masterGroupName}`,
      );
    ...
    if (!isActive) {
      ...
      sessionStorage.setItem(
        'globalFilterMasterGroup',
        JSON.stringify(globalFilterMasterGroup),
      );
...
function addShipAssignShareButton(
  ...
  shipAssignContainer.innerHTML = `
    ...
    '_',
      ...
      <button assign-group-data=${group.replace(
        / /g,
        '_',
      )} type="button" id="assign-product-button-${group.replace(
    / /g,
    '_',
  ...
  const assignProductBtn = shipAssignContainer.querySelector(
    `#assign-product-button-${group.replace(/ /g, '_')}`,
  );
  ...
  shareContainer.innerHTML = `
    ...
    <div>
      ...
      <button share-group-data=${group.replace(
        / /g,
        '_',
      )} type="button" id="share-product-button-${group.replace(
    / /g,
    '_',
  ...
  const shareProductBtn = shareContainer.querySelector(
    `#share-product-button-${group.replace(/ /g, '_')}`,
  );
