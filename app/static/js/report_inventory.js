...
var downloadCSV = function () {
    return __awaiter(this, void 0, void 0, function () {
        var searchInventoryInput, dateInventoryCreatedFromInput, dateInventoryCreatedToInput, filtersMap, filterQuery, _i, _a, _b, queryKey, queryInput, csvData, pages, queryTail, page, currentURL, urlWithoutQueryParams, url_1, res, data, blob, url, a;
        return __generator(this, function (_c) {
            switch (_c.label) {
                case 0:
                    searchInventoryInput = document.querySelector('#table-search-inventory');
                    dateInventoryCreatedFromInput = document.querySelector('#product-inventory-sort-created-from-datepicker');
                    dateInventoryCreatedToInput = document.querySelector('#product-inventory-sort-created-to-datepicker');
                    filtersMap = {
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var tableRow = document.querySelectorAll('.table-inventory-item-tr');
    tableRow.forEach(function (row) {
        var viewReportInventoriesModal = row.querySelector('.report-inventory-view-btn');
        var data = JSON.parse(viewReportInventoriesModal.getAttribute('data-target'));
        var reportStore = data.shipRequest.store.storeName;
        var reportInventoryStoreDiv = row.querySelector('.report-inventory-store');
        reportInventoryStoreDiv.innerHTML = reportStore;
    });
    var clearFilterButton = document.querySelector('#product-inventory-clear-button');
    ...
    buttonLoadInventoriesTable.click();
    // initialize modal
    var viewReportInventoriesModal = document.getElementById('view-report-inventories-modal');
    var viewModalOptions = {
        ...
        closable: true,
        onHide: function () {
            var productItems = document.querySelectorAll('.product-item-view');
            productItems.forEach(function (productItem) { return productItem.remove(); });
        },
    };
    var viewModal = new flowbite_1.Modal(viewReportInventoriesModal, viewModalOptions);
    var reportViewProductTbody = document.querySelector('#table-products');
    var productItemTemplate = document.querySelector('#view-product-item-template');
    var closingViewModalButton = document.querySelector('#button-closing-report-inventory-modal');
    closingViewModalButton.addEventListener('click', function () {
        viewModal.hide();
    });
    // view buttons click
    var reportViewUser = document.getElementById('report-inventory-user');
    var reportViewAction = document.getElementById('report-inventory-action');
    var reportViewDate = document.getElementById('report-inventory-date');
    var reportStoreName = document.getElementById('report-store-name');
    // onload element with inventories-table id
    htmxDispatcher.onLoad('inventories-table', function (target) {
        var reportViewButtons = target.querySelectorAll('.report-inventory-view-btn');
        reportViewButtons.forEach(function (btn) {
            btn.addEventListener('click', function () {
                ...
                reportInventory.reportInventories.forEach(function (inventory, i) {
                    // Render inventory
                    var newProductItem = productItemTemplate.cloneNode(true);
                    ...
                    var productIndex = newProductItem.querySelector('.product-index');
                    var productName = newProductItem.querySelector('.product-name');
                    var productSku = newProductItem.querySelector('.product-sku');
                    var productRegularPrice = newProductItem.querySelector('.product-regular-price');
                    var productRetailPrice = newProductItem.querySelector('.product-retail-price');
                    var productGroup = newProductItem.querySelector('.product-group');
                    var productWarehouse = newProductItem.querySelector('.product-warehouse');
                    var img = newProductItem.querySelector('.product-image');
                    inventory.product.image.length > 100
                        ? (img.src = "data:image/png;base64, ".concat(inventory.product.image))
                    ...
                    inventory.product.warehouseProducts.forEach(function (warehouseProduct) {
...
var reportViewAction = document.getElementById('report-inventory-action');
var reportViewDate = document.getElementById('report-inventory-date');
var reportStoreName = document.getElementById('report-inventory-store-name');
var productItemTemplate = document.querySelector('#report-inventory-view-product-item-template');
var reportViewProductTbody = document.querySelector('#report-inventory-table-products');
// initialize modal
var viewReportInventoriesModal = document.getElementById('view-report-inventories-modal');
var viewModalOptions = {
    placement: 'bottom-right',
    ...
    closable: true,
    onHide: function () {
        var productItems = document.querySelectorAll('.report-inventory-product-item-view');
        productItems.forEach(function (productItem) { return productItem.remove(); });
    },
};
var viewModal = new flowbite_1.Modal(viewReportInventoriesModal, viewModalOptions);
...
base_1.htmxLoader.onLoad('inventories-table', function (target) {
    var reportViewButtons = target.querySelectorAll('.report-inventory-view-btn');
    reportViewButtons.forEach(function (btn) {
        btn.addEventListener('click', function () {
            var reportInventory = JSON.parse(btn.getAttribute('data-target'));
            reportViewUser.innerHTML = reportInventory.user.username;
            reportViewAction.innerHTML = reportInventory.type;
            reportViewDate.innerHTML = (0, utils_1.formatDate)(reportInventory.createdAt);
            if (reportInventory.store) {
                reportStoreName.innerHTML = reportInventory.store.storeName;
            }
            else if (reportInventory.warehouse) {
            ...
            reportInventory.reportInventories.forEach(function (inventory, i) {
                ...
                newProductItem.removeAttribute('id');
                newProductItem.classList.remove('hidden');
                newProductItem.classList.add('report-inventory-product-item-view', 'text-base', 'font-semibold', 'text-gray-900', 'dark:text-white');
                var productIndex = newProductItem.querySelector('.product-index');
                var productName = newProductItem.querySelector('.product-name');
                var productSku = newProductItem.querySelector('.product-sku');
                var productRegularPrice = newProductItem.querySelector('.product-regular-price');
                ...
                inventory.product.image.length > 100
                    ? (img.src = "data:image/png;base64, ".concat(inventory.product.image))
                    : (img.src = base_1.defaultBrandImage);
                productIndex.innerHTML = (i + 1).toString();
                productName.innerHTML = inventory.product.name;
                productSku.innerHTML = inventory.product.SKU;
                if (inventory.qtyBefore) {
