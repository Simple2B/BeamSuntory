...
var $depleteProductModalElement = document.querySelector('#deplete-product-modal');
...
var depleteModal = new flowbite_1.Modal($depleteProductModalElement, depleteModalOptions);
...
var depleteProductButtonElements = document.querySelectorAll('.product-deplete-button');
depleteProductButtonElements.forEach(function (e) {
    return e.addEventListener('click', function () {
        var product = JSON.parse(e.getAttribute('data-target'));
        sessionStorage.setItem('product', JSON.stringify(product));
        var mstrGroups = Object.keys(product.mstr_groups_groups);
        mstrGroups.forEach(function (groupName) {
            ...
            createDepleteAction(isEqual, groupName, product.mstr_groups_groups[groupName], product);
        });
        var div = document.querySelector('#product-deplete-name');
        div.innerHTML = product.name;
        div = document.querySelector('#product-deplete-id');
        div.innerHTML = product.id.toString();
        var img = document.querySelector('#product-deplete-image');
        product.image.length > 100
            ? (img.src = "data:image/png;base64, ".concat(product.image))
            : (img.src = defaultBrandImage);
...
function requestShare(product, group) {
    ...
    input.value = product.SKU;
    input = document.querySelector('#product-request-share-group-id-hidden-input');
...
function addShipAssignShareButton(isEqual, masterGroup, group, productParam) {
    ...
    if (productParam.available_quantity[group] === 0 ||
...
function createDepleteAction(isEqual, masterGroup, group, productParam) {
    var groupProductIds = productParam.groups_ids;
    var productTypeContainer = document.querySelector("#product-deplete-product_type-container");
    var depleteContainer = document.createElement('div');
    depleteContainer.classList.add('sm:col-span-2', 'flex', 'gap-4');
    depleteContainer.setAttribute('id', "product-deplete-container-".concat(group.replace(/ /g, '_')));
    depleteContainer.innerHTML = "\n    <div>\n      <label for=\"deplete-product-quantity-".concat(group.replace(/ /g, '_'), "\" class=\"block mb-2 text-sm font-medium text-gray-900 dark:text-white\">Quantity</label>\n        <input id=\"deplete-product-quantity-").concat(group.replace(/ /g, '_'), "\"\n          class=\"shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500\">\n    </div>\n    <div>\n      <label for=\"product_group\" class=\"block mb-2 text-sm font-medium text-gray-900 dark:text-white\" >Action</label >\n      <button deplete-group-data=").concat(group.replace(/ /g, '_'), " type=\"button\" id=\"deplete-product-button-").concat(group.replace(/ /g, '_'), "\" class=\"deplete-product-button inline-flex items-center mr-2 px-3 py-2.5 text-sm font-medium text-center text-white rounded-lg bg-yellow-400 hover:bg-yellow-500 focus:ring-4 focus:ring-red-300 dark:focus:ring-red-900\">\n        <svg class=\"w-4 h-4 mr-2\" fill=\"currentColor\" viewBox=\"0 0 20 20\" xmlns=\"http://www.w3.org/2000/svg\"><path d=\"M17.414 2.586a2 2 0 00-2.828 0L7 10.172V13h2.828l7.586-7.586a2 2 0 000-2.828z\"></path><path fill-rule=\"evenodd\" d=\"M2 6a2 2 0 012-2h4a1 1 0 010 2H4v10h10v-4a1 1 0 112 0v4a2 2 0 01-2 2H4a2 2 0 01-2-2V6z\" clip-rule=\"evenodd\"></path></svg>\n        Deplete\n      </button>\n    </div>\n  ");
    var depleteProductBtn = depleteContainer.querySelector("#deplete-product-button-".concat(group.replace(/ /g, '_')));
    productTypeContainer.parentNode.insertBefore(depleteContainer, productTypeContainer.nextSibling);
    var productViewTypeContainer = document.querySelector('#product-deplete-product_type-container');
    var masterGroupWarehouseContainer = document.createElement('div');
    masterGroupWarehouseContainer.classList.add('sm:col-span-4');
    masterGroupWarehouseContainer.setAttribute('id', "product-deplete-product_group-container-".concat(group.replace(/ /g, '_')));
    masterGroupWarehouseContainer.innerHTML = "\n  <div class=\"flex gap-4\">\n  <div class=\"w-2/4\">\n    <label for=\"for-group-".concat(group, "\"\n      class=\"block mb-2 text-sm font-medium text-gray-900 dark:text-white\">").concat(masterGroup, "</label>\n    <select type=\"text\" name=\"group-").concat(group, "\" id=\"master-group-deplete-").concat(group, "\"\n      class=\"shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500\"\n      placeholder=\"Some Group\" required\n    >\n      <option value=\"").concat(groupProductIds[group], "\">").concat(group, "</option>\n    </select>\n  </div>\n  <div class=\"w-2/4\">\n    <label for=\"for-warehouse-").concat(group, "\"\n      class=\"block mb-2 text-sm font-medium text-gray-900 dark:text-white\">Warehouse</label>\n    <select type=\"text\" name=\"group-").concat(group, "\" id=\"warehouse-deplete-").concat(group, "\"\n      class=\"shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500\"\n      placeholder=\"Some Group\" required\n    >\n    </select>\n  </div>\n  </div>\n    ");
    var selectWarehouse = masterGroupWarehouseContainer.querySelector("#warehouse-deplete-".concat(group));
    var productQuantity = depleteContainer.querySelector("#deplete-product-quantity-".concat(group.replace(/ /g, '_')));
    ...
    var depleteButton = document.querySelector("#deplete-product-button-".concat(group.replace(/ /g, '_')));
    depleteButton.addEventListener('click', function () {
        ...
        depleteProduct(warehouseId, productId, currentQuantity, groupId, group, csrfToken);
    });
}
function depleteProduct(warehouseId, productId, quantity, groupId, group, csrfToken) {
    return __awaiter(this, void 0, void 0, function () {
        var data, response, message, groupContainer, productQuantity, successMessage_1, errorMessage_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    data = {
                    ...
                    return [4 /*yield*/, fetch('/product/deplete', {
                            method: 'POST',
                            headers: {
                ...
                case 2:
                    message = _a.sent();
                    groupContainer = document.querySelector("#product-deplete-product_group-container-".concat(group.replace(/ /g, '_')));
                    if (response.status === 201) {
                        productQuantity = document.querySelector("#deplete-product-quantity-".concat(group.replace(/ /g, '_')));
...
function deleteDepleteContainer(nameGroup, nameGroupValue) {
    var depleteContainer = document.querySelector("#product-deplete-container-".concat(nameGroupValue.replace(/ /g, '_')));
    var masterGroupWarehouseContainer = document.querySelector("#product-deplete-product_group-container-".concat(nameGroupValue.replace(/ /g, '_')));
    if (depleteContainer) {
