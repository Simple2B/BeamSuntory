...
if (filterData !== null || filterData !== undefined) {
    ...
    if (isVisibleFilter) {
        ...
        productItemTrs.forEach(function (product) {
            var referenceTd = product.cells[2];
            var productName = product.cells[1].innerText;
            for (var key in filterData) {
...
var showAllGroupsProductBtn = document.querySelector('#table-show-all-group-product-button');
showAllGroupsProductBtn.addEventListener('click', function () {
    var products = JSON.parse(showAllGroupsProductBtn.getAttribute('data-target'));
    console.log('products', products);
    var referenceTh = document.querySelector('#product-table-th-product-type');
    var productItemTrs = document.querySelectorAll('.table-product-item-tr');
    if (showAllGroupsProductBtn.classList.contains('show-all-groups-in-product')) {
        for (var _i = 0, _a = products.master_group_product_name; _i < _a.length; _i++) {
            var group = _a[_i];
            var isGroupExist = document.querySelector("#product-table-filter-master-group-".concat(group.replace(/ /g, '_')));
            if (isGroupExist) {
                isGroupExist.remove();
            }
        }
        productItemTrs.forEach(function (product) {
            ...
            for (var _i = 0, _a = products.master_group_product_name; _i < _a.length; _i++) {
                var group = _a[_i];
                var groupName = products[productName][group] || '-';
                var isGroupExist = document.querySelector("#product-table-filter-".concat(group.replace(/ /g, '_'), "-").concat(groupName.replace(/ /g, '_'), "-").concat(productName.replace(/ /g, '_')));
                if (isGroupExist) {
                    isGroupExist.remove();
                }
            }
        ...
        productItemTrs.forEach(function (product) {
            var productName = product.cells[1].innerText;
            for (var key in products[productName]) {
                var productFilterName = products[productName][key];
                var productFilterTd = document.querySelector("#product-table-filter-".concat(productFilterName.replace(/ /g, '_'), "-").concat(productName.replace(/ /g, '_')));
                if (productFilterTd) {
        ...
        for (var _b = 0, _c = products.master_group_product_name; _b < _c.length; _b++) {
            var group = _c[_b];
            var isGroupExist = document.querySelector("#product-table-filter-master-group-".concat(group.replace(/ /g, '_')));
            if (!isGroupExist) {
                var productFilterTh = document.createElement('th');
                productFilterTh.setAttribute('id', "product-table-filter-master-group-".concat(group.replace(/ /g, '_')));
                productFilterTh.classList.add('px-6', 'py-3');
                productFilterTh.setAttribute('scope', 'col');
        ...
        productItemTrs.forEach(function (product) {
            ...
            for (var _i = 0, _a = products.master_group_product_name; _i < _a.length; _i++) {
                ...
                var isGroupExist = document.querySelector("#product-table-filter-".concat(group.replace(/ /g, '_'), "-").concat(groupName.replace(/ /g, '_'), "-").concat(productName.replace(/ /g, '_')));
                if (!isGroupExist) {
                    var productFilterName = products[productName][group] || '-';
                    var productFilterTd = document.createElement('td');
                    productFilterTd.setAttribute('id', "product-table-filter-".concat(group.replace(/ /g, '_'), "-").concat(groupName.replace(/ /g, '_'), "-").concat(productName.replace(/ /g, '_')));
                    productFilterTd.classList.add('p-4', 'text-base', 'font-normal', 'text-gray-900', 'whitespace-nowrap', 'dark:text-white');
                    productFilterTd.innerHTML = "\n            <div class=\"pl-3\">\n              <div class=\"text-base font-semibold\">".concat(productFilterName, "</div>\n            </div>\n          ");
                    referenceTd.parentNode.insertBefore(productFilterTd, referenceTd.nextSibling);
                }
