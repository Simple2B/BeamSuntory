...
var $modalElement = document.querySelector('#editBillableGroupModal');
var addBillableGroupModalElement = document.querySelector('#add-billable-group-modal');
var modalOptions = {
...
var addModal = new flowbite_1.Modal(addBillableGroupModalElement, modalOptions);
var editGroupProductCloseModalButton = document.querySelector('#edit-billable-group-modal-close-btn');
editGroupProductCloseModalButton.addEventListener('click', function () {
    modal.hide();
});
var $buttonElements = document.querySelectorAll('.billable-group-edit-button');
...
// search flow
var searchInput = document.querySelector('#table-search-billable-groups');
var searchInputButton = document.querySelector('#table-search-billable-group-button');
if (searchInputButton && searchInput) {
    searchInputButton.addEventListener('click', function () {
        window.location.href = "".concat(window.location.origin).concat(window.location.pathname, "?q=").concat(searchInput.value);
    });
}
var deleteButtons = document.querySelectorAll('.delete-billable-group-btn');
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    id = e.getAttribute('data-billable-group-id');
                    return [4 /*yield*/, fetch("/billable_group/delete/".concat(id), {
                            method: 'DELETE',
                        })];
                case 1:
                    response = _a.sent();
...
function editBillableGroup(billableGroup) {
    var input = document.querySelector('#billable-group-edit-name');
    ...
    var select = document.querySelector('#master_billable_group_id');
    select.value = billableGroup.master_billable_group_id.toString();
    modal.show();
}
// for multiple creating
var deleteAllocatedGroup = function (e) {
    var groupAllocatedContainer = e.currentTarget.parentNode;
    var groupsAllocatedContainer = groupAllocatedContainer.parentNode;
    groupAllocatedContainer.remove();
    if (groupsAllocatedContainer.children.length == 2) {
        var productAllocatedDeleteButton = groupsAllocatedContainer.querySelector('.group-allocated-delete-button');
        productAllocatedDeleteButton.classList.add('invisible');
    }
};
var createAllocationBillableGroupContainer = function (e) {
    var btn = e.currentTarget;
    var groupAllocatedContainer = document.querySelector('.group-allocated').parentNode;
    if (groupAllocatedContainer.parentNode.children.length == 2) {
        var buttonRemoveGroupAllocated = groupAllocatedContainer.querySelector('.group-allocated-delete-button');
        buttonRemoveGroupAllocated.classList.remove('invisible');
    }
    var groupAllocatedNew = groupAllocatedContainer.cloneNode(true);
    // Clear inputs
    groupAllocatedNew.querySelectorAll('input').forEach(function (input) {
        input.value = '';
    });
    var buttonDeleteAllocatedGroup = groupAllocatedNew.querySelector('.group-allocated-delete-button');
...
var createMultipleBillableGroupsHandler = function () {
    var createBillableGroupsBtn = document.getElementById('billable-groups-create-btn');
    var createBillableGroupsBtnSubmit = document.getElementById('billable-groups-add-submit-btn');
    if (!createBillableGroupsBtn) {
        console.log('Error: no create inbound order button');
        return;
    }
    createBillableGroupsBtn.addEventListener('click', function () {
        var allocatedGroupsData = [];
        // Set products as JSON to field
        var groupsAllocatedContainers = document.querySelectorAll('.group-allocated');
        groupsAllocatedContainers.forEach(function (groupContainer) {
            // Get HTML nodes with product values
            var groupAllocatedNameInput = groupContainer.querySelector('.group-allocated-name');
            var groupAllocatedRateInput = groupContainer.querySelector('.group-allocated-rate');
            var groupAllocatedAssignToInboundCheckbox = groupContainer.querySelector('.group-allocated-assign-to-inbound');
            var groupAllocatedAssignToOutboundCheckbox = groupContainer.querySelector('.group-allocated-assign-to-outbound');
            console.log(groupAllocatedAssignToInboundCheckbox, groupAllocatedAssignToOutboundCheckbox);
            // Retrieve values from Nodes
            var groupAllocatedName = groupAllocatedNameInput.value;
            var groupAllocatedRate = parseFloat(groupAllocatedRateInput.value);
            var groupAllocatedAssignToInbound = groupAllocatedAssignToInboundCheckbox.checked;
            var groupAllocatedAssignToOutbound = groupAllocatedAssignToOutboundCheckbox.checked;
            console.log(groupAllocatedName, groupAllocatedRate, groupAllocatedAssignToInbound, groupAllocatedAssignToOutbound);
        ...
        var inputGroups = document.querySelector("#billable-add-many-add-groups");
...
document.addEventListener('DOMContentLoaded', function () {
    var buttonAllocateProduct = document.getElementById('billable-groups-allocate-btn');
    buttonAllocateProduct.addEventListener('click', createAllocationBillableGroupContainer);
    var buttonDeleteAllocatedProduct = document.querySelector('.group-allocated-delete-button');
