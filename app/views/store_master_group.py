...
master_group_blueprint = Blueprint(
    "store_master_group", __name__, url_prefix="/store-master-group"
)
...
def get_all():
    ...
    query = sa.select(m.StoreMasterGroup).order_by(m.MasterGroup.name.asc())
...
@master_group_blueprint.route("/get-create-modal", methods=["GET"])
@login_required
@role_required([s.UserRole.ADMIN.value])
def get_create_modal():
    form = f.NewMasterGroupForm()
    return render_template("master_group/create_master_group.html", form=form)
